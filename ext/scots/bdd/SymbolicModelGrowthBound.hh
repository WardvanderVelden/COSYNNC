  ...
  public:
    ...
    void computeTransitionRelation(F1 &system_post, F2 &radius_post) {
      ...
      for(begin(); !done(); next()) {
        ...
        //std::cout << " x0': " << x[0] << " x1': " << x[1] << " r0: " << r[0] << " r1: " << r[1] << std::endl;
        ...
        if(!(post==ddmgr_->bddZero()) && post<= constraints) {
          ...
          //std::cout << std:: endl << "current: " << current << " post: " << post << std::endl;

          current&=post;
          transitionRelation_ +=current;

          // WARD:  Hence the transition relation is a BDD containing BDD(x) & BDD(u) & BDD(X')
          //        where BDD(X') is the set of indices that are in the post
